...
- Codex app-server 的有界 stdio transport、RPC broker、typed thread/turn client、
  ...
  服务端进程的有界只读 WebSocket transport；
...
外部端点已有 fail-closed 准入和只读长连接 transport，但重连、订阅恢复和共享写入仍未
接入普通 `run` 链路；选择 external mode 不会回退为新起一个 stdio child。配置与验收说明见
[`docs/external-codex-endpoint-gate.md`](docs/external-codex-endpoint-gate.md) 和
[`docs/external-codex-transport.md`](docs/external-codex-transport.md)。
