...
const domain = createStableDomain({
  ...
  allowFunctionCallOutput: true,
});

const FILTERED_0_151_NOTIFICATIONS = Object.freeze([
...
function initializeRequest(params) {
  const projected = domain.initializeRequest(params);
  const capabilities =
    projected.capabilities === undefined || projected.capabilities === null
      ? {}
      : projected.capabilities;
  const requested = capabilities.optOutNotificationMethods ?? [];
  capabilities.optOutNotificationMethods = [
    ...new Set([...requested, ...FILTERED_0_151_NOTIFICATIONS]),
  ];
  return { ...projected, capabilities };
}

// This is an independent 0.151.0 mapping table. It intentionally does not
// inherit the 0.149.0 adapter: additive provider DTOs must be normalized here
// before they can cross the stable Rust boundary.
