...
Codex 只走 sidecar，精确接受 `codex-cli 0.149.0` 或 `0.151.0`。确认 Node 20+、
`codex-sidecar/index.cjs` 存在，并且已经 `npm ci --ignore-scripts --prefix codex-sidecar`。

覆盖 pinned Codex 或使用非默认 Node、Codex home、wrapper 参数时，同步传入
`--codex-binary`、`--node-binary`、`--codex-home` 和可重复的 `--codex-argument`：
...
Codex stderr、用户正文或完整路径。sidecar 失败不会切到 stdio。修复后重新
...
普通 `run` 不会再走 `spawned_stdio` 或 `external_endpoint`。要独立复核 admission
...
如果原因是 unsupported version，sidecar 只认精确的 0.149.0/0.151.0。其他版本必须先加审核过的
adapter，不能仅因本机版本号更高就绕过门禁。
