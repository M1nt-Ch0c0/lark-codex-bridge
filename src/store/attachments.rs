...
impl StoreHandle {
    ...
    /// Reads the persisted reconciliation scan cursor: the name of the last
    /// directory entry fully processed by the previous truncated pass, or
    /// `None` when the next pass should start from the head of the directory.
    ///
    /// # Errors
    ///
    /// Returns an error when the writer task or SQLite fails.
    pub async fn attachment_scan_cursor(&self) -> Result<Option<String>, StoreError> {
        self.run(|connection| {
            query_optional(
                connection.query_row(
                    "SELECT entry_name FROM attachment_scan_cursor WHERE id = 1",
                    [],
                    |row| row.get(0),
                ),
                "reading the attachment scan cursor",
            )
        })
        .await
    }

    /// Persists the reconciliation scan cursor after a truncated pass. The
    /// single-row table is upserted, so repeated truncated passes overwrite
    /// the previous cursor instead of growing the table.
    ///
    /// # Errors
    ///
    /// Returns an error when the writer task or SQLite fails.
    pub async fn set_attachment_scan_cursor(&self, entry_name: &str) -> Result<(), StoreError> {
        let entry_name = entry_name.to_owned();
        let request_size = request_bytes(&[&entry_name]);
        self.run_sized(request_size, move |connection| {
            connection
                .execute(
                    "INSERT INTO attachment_scan_cursor (id, entry_name) VALUES (1, ?1)
                     ON CONFLICT (id) DO UPDATE SET entry_name = excluded.entry_name",
                    params![entry_name],
                )
                .map_err(|error| sqlite_error("saving the attachment scan cursor", &error))?;
            Ok(())
        })
        .await
    }

    /// Clears the reconciliation scan cursor once a pass has reached the end
    /// of the directory, so the next pass starts a fresh scan from the head.
    ///
    /// # Errors
    ///
    /// Returns an error when the writer task or SQLite fails.
    pub async fn clear_attachment_scan_cursor(&self) -> Result<(), StoreError> {
        self.run(|connection| {
            connection
                .execute("DELETE FROM attachment_scan_cursor WHERE id = 1", [])
                .map_err(|error| sqlite_error("clearing the attachment scan cursor", &error))?;
            Ok(())
        })
        .await
    }
