...
pub(crate) fn is_documented_transient(code: i64) -> bool {
    code == 429 || code == 99_991_400
}

pub(crate) fn is_http_server_error(code: i64) -> bool {
...
async fn record_final_card_patch_failure(
    ...
    if error.patch_retryable() && attempts < STORE_OUTBOX_MAX_ATTEMPTS {
    ...
    if error.class() == DeliveryFailureClass::Uncertain {
    ...
    if error.class() == DeliveryFailureClass::Retryable && attempts < STORE_OUTBOX_MAX_ATTEMPTS {
        return record_failure(
            store,
            row,
            DeliveryClass::Retryable,
...
async fn record_card_patch_failure(
    ...
    let class = if error.class() == DeliveryFailureClass::Uncertain {
        DeliveryClass::Uncertain
    } else {
        DeliveryClass::Permanent
    };
